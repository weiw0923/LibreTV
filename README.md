...
这是最简部署方式，一个文件搞定，无需构建工具。

1. 打开 [worker.js](worker.js)，全选复制内容
2. 登录 [Cloudflare Dashboard](https://dash.cloudflare.com/) → Workers 和 Pages
3. 点击"创建" → "Worker"，粘贴代码
4. 在 **设置 → 变量** 中添加：
   - `PASSWORD` = `your_password`（自定义密码）
5. 保存并部署
6. （可选）添加自定义域名：Worker 设置 → 触发器 → 自定义域
...
- **Cloudflare Worker**: 在 Worker 设置 → 变量中添加 `PASSWORD`
- **Cloudflare Pages / Vercel**: 在项目设置 → 环境变量中添加 `PASSWORD`
